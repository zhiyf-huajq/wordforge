...
**声明：纯用ai搓着玩的。** 这是个个人练习项目（也是我第一个开源出来的东西），**不是成熟产品**，没有团队在维护。源码、注释、这份说明里里外外都是 AI 写的，我自己也是边做边学。**背单词的数据和算法没经过专业校验，别拿它当唯一的学习依据，也别拿它当 AI 写代码的范本。** 功能会随时改，不保证向后兼容。能用、能改、出问题自己动手 —— 就这样。
...
改完代码不是「看着没问题」就算完。这个项目有九层验证，每层跑出可复现的数字：

| 层 | 手段 | 结果 |
|---|---|---|
| 结构 | 静态扫描：标签配对、网络调用白名单、外部资源、数据质量 | **0 错误 / 1 警告**（标签计数对单文件应用有噪声，见下）|
| 逻辑 | 纯 Node 假 DOM（**不起浏览器**）跑 718 条断言（算法 / 路由 / 出题 / 边界） | **718 / 718** |
...
| 端到端 | 真实网络：中国日报走 CORS 中转、环球时报直连取回正文 | **15 / 15** |
...
- 结构层那 **1 条警告**是 `<div>` 计数：检查脚本按文本数标签，会把 JS 模板字符串里跨行拼接的 `'<div…' + "…</div>"` 也数进去，必然「开一半闭一半」。**基线产物同样如此**，不是真缺陷 —— 判断有没有未闭合标签得先渲染出 HTML 再用栈式解析器配对，拿计数下结论会误判。
...
_dev/           构建脚本、ETL 脚本、九层验证脚本、发版脚本，以及精读用的真实网页夹具
dist/           构建产物：单文件 HTML
preview/        界面截图
...
**纯用ai搓着玩的** —— 这个项目**始于兴趣，也止于兴趣**。

- **不是产品**：没有服务承诺，没有更新计划，不保证修 bug、不保证向后兼容。今天能用，明天我可能就改崩了。
- **AI 生成比例极高**：从应用代码、构建脚本、测试到这份 README，几乎全部由 AI 产出。我在里面做的是提需求、试效果、挑毛病。（顺带一提，用 AI 写一个「用 AI 写的东西」，本身就挺玩票的。）
- **数据与算法请自行甄别**：词条释义、例句、音标、记忆曲线参数都是自动整理来的，**可能有错**，遇到可疑的地方以权威词典和老师讲的为准。
- **不是范本**：想学怎么组织结构、怎么管 CI，可能会有点参考价值；想学代码怎么写得漂亮，别看我这个。
- **责任自负**：按 MIT 协议原样提供，用出任何问题自己负责。
